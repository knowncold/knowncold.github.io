...
## M0 Pro上的串口  
Arduino M0 Pro有两个USB端口，`Native port`(通过SerialUSB对象来支持CDC虚拟通信)直接连在SAMD21单片机上，另一个`Programming port`则是连接在ATMEL embedded debugger (EDBG)，作为板上的编程器和调试器同时扮演着USB转串口的角色，Programming port是默认的用来烧写代码和通信的端口。
